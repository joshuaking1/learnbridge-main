...
import { Loader2, Camera, RotateCw } from 'lucide-react';
    ...
    return (
        <div className="w-full">
            <div className="bg-gradient-to-r from-brand-orange/10 to-brand-orange/5 p-6 border-b flex justify-between items-center">
                <div>
                    <CardTitle className="text-2xl text-gray-800 mb-2">Profile Settings</CardTitle>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        <div className="flex flex-col items-center space-y-4 md:border-r md:pr-6">
                            <Avatar className="h-32 w-32 border-4 border-white shadow-lg">
                                <AvatarImage
                                    src={(profileData?.profile_image_url || user?.profile_image_url) || undefined}
                                    alt={(profileData?.first_name || user?.first_name) || 'User'}
                                />
                                <AvatarFallback className="bg-brand-orange text-white text-2xl">
                                    {(profileData?.first_name?.[0] || user?.first_name?.[0])?.toUpperCase() || 'U'}
                                </AvatarFallback>
                            </Avatar>
                            <div className="text-center mt-4">
                                <h3 className="text-xl font-semibold text-gray-800">
                                    {profileData?.first_name || user?.first_name || user?.firstName || ''} {profileData?.surname || user?.surname || ''}
                                </h3>
                                <p className="text-gray-500 capitalize">{profileData?.role || user?.role}</p>
                            </div>
                            <div className="mt-4">
                                <Input
                                    type="file"
                                ...
                                <div className="grid gap-2">
                                    ...
                                    <div className="flex space-x-2">
                                        <div className="flex-1">
                                            <Input
                                                value={profileData?.first_name || user?.first_name || user?.firstName || ''}
                                                disabled
