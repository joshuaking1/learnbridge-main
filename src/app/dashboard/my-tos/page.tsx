...
export default function MyTosPage() {
  ...
  const handleDeleteTos = async (tosId: number) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://learnbridge-teacher-tools-service.onrender.com/api/teacher-tools/tos/${tosId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      toast({
        title: "Success",
        description: "Table of Specifications deleted successfully.",
      });
    } catch (error: any) {
      console.error("Error deleting ToS:", error);
      toast({
        title: "Delete Error",
        description: error.message || "Could not delete ToS.",
        variant: "destructive",
      });
    } finally {
      setDeletingId(null);
    ...
    return (
      <div className="min-h-screen bg-slate-100 p-4 md:p-8">
        <Alert variant="destructive">
          ...
          <AlertDescription>
            This page is only accessible to teachers. Please log in with a
            teacher account.
          </AlertDescription>
        </Alert>
      </div>
    ...
    <div className="min-h-screen bg-slate-100 p-4 md:p-8">
        ...
        <div className="flex justify-center items-center py-16">
          <div className="text-center">
            ...
            <p className="text-slate-400">
              Loading your tables of specification...
            </p>
          </div>
        </div>
      )}
      ...
      {!isLoadingTos && errorLoading && (
        <Alert variant="destructive" className="mt-6">
          <AlertTitle>Error Loading Tables of Specification</AlertTitle>
      ...
      {!isLoadingTos && !errorLoading && tablesOfSpecs.length === 0 && (
        <Card className="text-center py-10 mt-6">
          <CardHeader>
            <CardTitle>No Saved ToS Yet</CardTitle>
          </CardHeader>
          <CardContent>
            <CardDescription className="mb-4">
              Generate a Table of Specifications using the AI ToS Builder!
            </CardDescription>
            ...
            >
              <CardHeader>
                <CardTitle className="text-lg truncate">
                  {tos.title ||
                    `ToS: ${
                      tos.assessment_title?.substring(0, 30) ?? "Untitled"
                    }...`}
                </CardTitle>
                <CardDescription>
                  {tos.subject} - {tos.book}
                </CardDescription>
              </CardHeader>
              <CardContent className="flex-grow">
                <p className="text-sm text-gray-600 line-clamp-2">
                  For: {tos.assessment_title}
                </p>
                <p className="text-xs text-gray-400 mt-2">
                  Last Updated:{" "}
                  {tos.updated_at
                    ? format(new Date(tos.updated_at), "PPp")
                    : "N/A"}
                </p>
              </CardContent>
              <CardFooter className="flex justify-end space-x-2 border-t pt-4">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  title="View or Edit ToS"
                  onClick={() =>
                    alert(
                      `View/Edit ToS ID: ${tos.id} - Page not implemented yet.`
                    )
                  }
                >
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        This will permanently delete the Table of Specs for:{" "}
                        <br />
                        <strong className="break-words">
                          {tos.title || tos.assessment_title}
                        </strong>
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
