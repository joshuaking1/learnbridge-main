...
const tosSchema = z.object({
  ...
  assessmentTitle: z
    .string()
    .min(5, { message: "Assessment Title is required." }),
...
export default function TosBuilderPage() {
  ...
  const handleCopy = useCallback(() => {
    if (!generatedTos) {
      toast({
        ...
        description: "Generate a Table of Specifications first.",
        variant: "destructive",
      });
      return;
    }
    ...
    navigator.clipboard
      .writeText(generatedTos)
      .then(() => {
        toast({
          title: "Copied!",
          description: "Table of Specifications copied to clipboard.",
        });
      })
      .catch((err) => {
  ...
  async function onSubmit(values: TosFormValues) {
    ...
    try {
      ...
      const response = await fetch("/api/ai/generate/tos", {
        method: "POST",
        headers: {
      ...
      if (response.status === 429) {
        ...
        toast({
          ...
          description:
            "You've reached your daily usage limit for this service.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (!response.ok) {
        toast({
          ...
          description:
            "Error generating Table of Specifications. Please try again.",
          variant: "destructive",
        });
        return;
      }
  ...
  const handleSaveTos = async () => {
    ...
    const coveredTopics = currentInputs.coveredTopicsString
      ? currentInputs.coveredTopicsString
          .split("\n")
          .map((t) => t.trim())
          .filter((t) => t.length > 0)
      : [];
    ...
    try {
      const response = await fetch(
        "https://learnbridge-teacher-tools-service.onrender.com/api/teacher-tools/tos",
        {
          // Use new ToS endpoint
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
      ...
      if (!response.ok) {
        ...
        toast({
          ...
          description:
            "Error saving Table of Specifications. Please try again.",
          variant: "destructive",
        });
        return;
      }
      ...
      toast({
        title: "Network Error",
        description: "Could not connect to server to save ToS.",
        variant: "destructive",
      });
    } finally {
      setIsSaving(false);
      ...
      <header className="mb-6 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-brand-darkblue">
            AI Table of Specifications Builder
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          {limitReached && (
            <Alert
              variant="destructive"
              className="mb-4 bg-red-600 text-white border-red-700 shadow-md"
            >
              ...
              <AlertDescription className="text-white">
                You've reached your daily usage limit for the ToS Builder. The
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Provide the details for the assessment blueprint.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form}>
                ...
                >
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="e.g., Art and Design Studio"
                            {...field}
                  ...
                  <FormField
                    control={form.control}
                    name="coveredTopicsString"
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
                          Specific Topics/Strands Covered (Optional)
                        </FormLabel>
                        <FormControl>
                          <Textarea
                        ...
                        <FormDescription>
                          If blank, the AI will attempt to cover all relevant
                          topics from the selected book.
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          Adjust the balance between objective and subjective
                          questions.
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
        ...
        <Card className="lg:col-span-2" id="generated-tos-section">
          <CardHeader>
            ...
            <CardDescription>
              Review the AI-generated assessment blueprint below.
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            {!isGenerating && !generatedTos && (
              <div className="text-center text-gray-500 h-40 flex items-center justify-center">
                Enter details and click "Generate" to see the Table of Specs
                here.
              </div>
            )}
                ...
                <div className="prose prose-sm sm:prose-base max-w-none">
                  <ReactMarkdown
                    ...
                    components={{
                      table: ({ node, ...props }) => (
                        <table
                          className="table-auto w-full border-collapse border border-slate-400 my-4"
                          {...props}
                        />
                      ),
                      thead: ({ node, ...props }) => (
            ...
            <div className="flex flex-col space-y-2">
              ...
              {saveSuccess === false && (
                <Alert variant="destructive" className="w-full max-w-xs">
                  ...
                  <AlertDescription>
                    There was an error saving the Table of Specifications.
                    Please try again.
                  </AlertDescription>
                </Alert>
              )}
              {saveSuccess === true && (
                <Alert
                  variant="success"
                  className="w-full max-w-xs bg-green-100 border-green-300 text-green-800"
                >
