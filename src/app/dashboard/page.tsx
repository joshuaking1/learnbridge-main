...
import { AuthChangeBanner } from "@/components/auth/AuthChangeBanner";
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            <Link href="/dashboard/lesson-planner" className="block">
              <div className="bg-white/10 backdrop-blur-sm p-4 sm:p-6 rounded-lg border border-white/20 hover:bg-white/20 transition-all">
                <h2 className="text-lg sm:text-xl font-arvo font-bold text-white mb-2">
                  Lesson Planner
                </h2>
            ...
            <Link href="/dashboard/assessment-creator" className="block">
              <div className="bg-white/10 backdrop-blur-sm p-4 sm:p-6 rounded-lg border border-white/20 hover:bg-white/20 transition-all">
                ...
                <p className="text-white/80 text-sm sm:text-base">
                  Generate assessments and quizzes for your students
                </p>
              </div>
            </Link>
            ...
            <Link href="/dashboard/rubric-generator" className="block">
              <div className="bg-white/10 backdrop-blur-sm p-4 sm:p-6 rounded-lg border border-white/20 hover:bg-white/20 transition-all">
                ...
                <p className="text-white/80 text-sm sm:text-base">
                  Create grading rubrics for your assessments
                </p>
              </div>
            </Link>
            ...
            <Link href="/dashboard/tos-builder" className="block">
              <div className="bg-white/10 backdrop-blur-sm p-4 sm:p-6 rounded-lg border border-white/20 hover:bg-white/20 transition-all">
                ...
                <p className="text-white/80 text-sm sm:text-base">
                  Generate a Table of Specification to align content with
                  learning goals and balanced assessment coverage.
                </p>
              </div>
            </Link>
          </div>
        ...
        <div className="mt-6 sm:mt-8 grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          ...
          <Link href="/dashboard/usage-limits" className="block">
            <div className="bg-white/10 backdrop-blur-sm p-4 sm:p-6 rounded-lg border border-white/20 hover:bg-white/20 transition-all h-full">
              ...
              <p className="text-white/80 text-sm sm:text-base">
                Monitor your daily usage limits across all services
              </p>
            </div>
          </Link>
        </div>
          ...
          <div className="mt-6 sm:mt-8 space-y-4 sm:space-y-6">
            <div className="p-6 bg-white/10 backdrop-blur-sm rounded-lg shadow-lg border border-white/20 hover:bg-white/15 transition-all space-y-3">
              ...
              <p className="text-slate-200 ml-11">
                Access curriculum content, track your progress, and get AI help
                with your studies.
              </p>
        ...
        {userRole === "admin" && (
          <div className="mt-6 sm:mt-8 space-y-4 sm:space-y-6">
            <div className="p-4 sm:p-6 bg-white border rounded-lg shadow space-y-3">
              <h3 className="text-lg sm:text-xl font-semibold text-brand-orange">
                Admin Tools
              </h3>
              <div className="flex flex-wrap gap-2">
                ...
                <Link href="/dashboard/admin/daily-quizzes">
                  <Button variant="secondary" className="w-full sm:w-auto">
...
function StudentContentViewer() {
  ...
  const [errorLoadingContent, setErrorLoadingContent] = useState<string | null>(
    null
  );
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Could not load learning material.";
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Select your subject and grade to find relevant SBC content.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <Button
          ...
          disabled={!selectedSubject || !selectedGrade || isLoadingContent}
          ...
          {!isLoadingContent &&
            ...
            contentChunks.length === 0 && (
              <p className="text-center text-gray-500 py-6">
                Select subject and grade, then click &quot;Load Material&quot;.
                No content found for the current selection.
              </p>
            )}
          {!isLoadingContent &&
            !errorLoadingContent &&
            contentChunks.length > 0 && (
              <ScrollArea className="h-[40vh] sm:h-[50vh] p-3 sm:p-4 border rounded-md bg-white text-black">
                {" "}
                {/* Scrollable content */}
                <h3 className="text-base sm:text-lg font-semibold mb-3 text-brand-darkblue">
                  Relevant Content:
                </h3>
                {contentChunks.map((chunk) => (
                  ...
                  >
                    <p className="text-xs text-gray-500 mb-1">
                      Source: {chunk.source_document_name || "Unknown"} (Chunk{" "}
                      {chunk.chunk_index})
                      {chunk.similarity &&
                        ` | Similarity: ${(chunk.similarity * 100).toFixed(
                          1
                        )}%`}
                    </p>
